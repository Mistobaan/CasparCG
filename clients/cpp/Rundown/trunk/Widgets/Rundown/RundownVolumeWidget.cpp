...
bool RundownVolumeWidget::executeCommand(enum Playout::PlayoutType::Type type)
{
...
void RundownVolumeWidget::executePlay()
{
    const QSharedPointer<CasparDevice> device = DeviceManager::getInstance().getConnectionByName(this->model.getDeviceName());
    if (device != NULL && device->isConnected())
        device->setVolume(this->command.getChannel(), this->command.getVideolayer(), this->command.getVolume(),
...
void RundownVolumeWidget::checkGpiTriggerable()
{
...
void RundownVolumeWidget::gpiDeviceConnected(bool connected, GpiDevice* device)
{
    checkGpiTriggerable();
}
