...
bool LibraryWidget::eventFilter(QObject* target, QEvent* event)
{
    if (event->type() == static_cast<QEvent::Type>(Enum::EventType::MediaChanged))
    {
        ...
        this->toolBoxLibrary->setItemText(1, QString("Image (%1)").arg(this->treeWidgetImage->topLevelItemCount()));
        this->toolBoxLibrary->setItemText(3, QString("Video (%1)").arg(this->treeWidgetVideo->topLevelItemCount()));
    }
    else if(event->type() == static_cast<QEvent::Type>(Enum::EventType::TemplateChanged))
    {
        ...
        this->toolBoxLibrary->setItemText(2, QString("Template (%1)").arg(this->treeWidgetTemplate->topLevelItemCount()));
    }
